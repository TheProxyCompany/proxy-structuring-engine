...
def test_bias_logits_empty_accepted_tokens(array_type):
    ...
    biased_logits = get_logit_bias(logits, accepted_token_ids)
    if lib_name == "torch":
        assert isinstance(biased_logits, torch.Tensor)
        assert torch.allclose(
            biased_logits, logits
        ), "Biased logits should be equal to original logits"
    elif lib_name == "jax":
...
def test_bias_logits_some_accepted_tokens(array_type):
    ...
    expected = [0.0, float("-inf"), 0.0, float("-inf"), 0.0]
    expected_biased_logits = array_constructor(expected)

    if lib_name == "torch":
        assert isinstance(biased_logits, torch.Tensor)
        assert torch.allclose(
            biased_logits, expected_biased_logits, equal_nan=True
        ), "Biased logits do not match expected values"
    elif lib_name == "jax":
        assert isinstance(biased_logits, jnp.ndarray)
        assert jnp.allclose(
            biased_logits, expected_biased_logits, equal_nan=True
        ...
        assert isinstance(biased_logits, mx.array)
        assert mx.allclose(
            biased_logits, expected_biased_logits
        ), "Biased logits do not match expected values"
    else:
        assert isinstance(biased_logits, np.ndarray)
        assert np.allclose(
...
def test_bias_logits_empty_logits(array_type):
    ...
    assert (
        biased_logits.shape == logits.shape
    ), "Biased logits should have the same shape as logits"
...
def test_bias_logits_logits_with_inf(array_type):
    ...
    expected = [0.0, 0.0, float("-inf"), float("-inf"), 0.0]
    expected_biased_logits = array_constructor(expected)
...
def test_bias_logits_logits_with_different_dtypes(array_type):
    ...
    for dtype in dtypes:
        ...
        if lib_name == "torch":
            assert isinstance(biased_logits, torch.Tensor)
            assert torch.allclose(
                biased_logits, expected_biased_logits, atol=1e-8
            ), f"Biased logits do not match expected values for dtype {dtype}"
        elif lib_name == "jax":
            assert isinstance(biased_logits, jnp.ndarray)
            assert jnp.allclose(
                biased_logits, expected_biased_logits, atol=1e-8
