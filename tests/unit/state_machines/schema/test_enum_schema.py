...
class TestEnumSchemaAcceptor(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.state_machine: EnumSchemaStateMachine = EnumSchemaStateMachine(
    ...
    def test_accept_valid_enum_value(self) -> None:
        ...
        for char in valid_value:
            walkers = [
                walker
                for _, walker in EnumSchemaStateMachine.advance_all(walkers, char)
            ]
    ...
    def test_accept_multiple_enum_values(self) -> None:
        ...
        for valid_value in valid_values:
            with self.subTest(valid_value=valid_value):
                ...
                walkers = [
                    walker
                    for _, walker in EnumSchemaStateMachine.advance_all(
                        walkers, valid_value
                    )
                ]
                for walker in walkers:
    ...
    def test_init_without_enum_key(self) -> None:
        ...
        with self.assertRaises(KeyError):
            EnumSchemaStateMachine(schema=schema)
    ...
    def test_accept_empty_string_when_enum_is_empty(self) -> None:
        ...
        """
        schema: dict = {"enum": []}
        state_machine: EnumSchemaStateMachine = EnumSchemaStateMachine(schema=schema)
