...
def test_basic_wait_for_acceptor() -> None:
    ...
    walkers = [
        walker
        for _, walker in
        acceptor.advance_all(walkers, "Hello World")
    ]
    assert len(walkers) == 1
...
def test_interrupted_wait_for_acceptor() -> None:
    ...
    walkers = [
        walker
        for _, walker in
        acceptor.advance_all(walkers, "Hello ")
    ]
    assert len(walkers) == 1
    ...
    walkers = [
        walker
        for _, walker in
        acceptor.advance_all(walkers, "I'm gonna mess up the pattern! But i'll still be accepted!")
    ]
    assert len(walkers) == 1
