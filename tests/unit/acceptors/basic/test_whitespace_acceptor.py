...
def test_whitespace_acceptor_various_whitespace_characters(input_str, expected_value):
...
def test_whitespace_acceptor_partial_whitespace_input():
    ...
    assert not any(cursor.in_accepted_state() for cursor in cursors)
    assert any(cursor.get_value() == "  " and cursor.remaining_input == "a" for cursor in cursors)
