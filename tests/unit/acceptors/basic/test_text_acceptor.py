...
def test_advance_incomplete(text_acceptor: TextAcceptor):
    ...
    walker = text_acceptor.walker_class(text_acceptor, consumed_character_count=0)
    for walker in walker.consume_token("h"):
        assert walker.accumulated_value() == "h👉ello"
...
def test_advance_complete(text_acceptor: TextAcceptor):
    ...
    walker = text_acceptor.walker_class(
        text_acceptor, consumed_character_count=4
    )  # Position before last character
    advanced = list(walker.consume_token("o"))
    assert len(advanced) == 1
    accepted = advanced[0]
    assert isinstance(accepted, AcceptedState)
    assert accepted.accumulated_value() == "hello"
...
def test_repeated_characters():
    ...
    walker = acceptor.walker_class(acceptor, consumed_character_count=0)
    for char in repeated_text:
        advanced = list(walker.consume_token(char))
        assert len(advanced) == 1
        walker = advanced[0]
    assert isinstance(walker, AcceptedState)
    assert walker.accumulated_value() == repeated_text
...
def test_unicode_characters():
    ...
    for char in unicode_text:
        ...
        walker = advanced[0]
    assert isinstance(walker, AcceptedState)
    assert walker.accumulated_value() == unicode_text
...
def test_invalid_initial_position(text_acceptor: TextAcceptor):
    ...
    walker = text_acceptor.walker_class(text_acceptor, consumed_character_count=-1)
