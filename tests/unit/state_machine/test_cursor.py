...
class ConcreteCursor(Cursor):
    ...
    def advance(self, input: str) -> Iterable[Cursor]:
        ...
        self.consumed_character_count += len(input)
...
class MockTokenAcceptor(TokenAcceptor):
    ...
    def advance_cursor(self, cursor: Cursor, input: str) -> Iterable[Cursor]:
        return super().advance_cursor(cursor, input)
...
def test_cursor_advance(cursor: ConcreteCursor):
    ...
    input_str = "input"
    cursors = list(cursor.advance(input_str))
    assert len(cursors) == 1
    advanced_cursor = cursors[0]
    assert advanced_cursor.consumed_character_count == len(input_str)
